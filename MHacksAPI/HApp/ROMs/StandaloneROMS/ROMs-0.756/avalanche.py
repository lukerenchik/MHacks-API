...
#a line for the rom to let main know if it is halted or not
romHaltAddress = id([False])
#value to pause rom
pauseRomAddress = id([False])
#value to end rom
endRomAddress = id([False])
...
class romTemplate:
    ...
    def __init__(self, api, difficultySetting, debug, difficultyRate, baseFallTime, controller):
        """ initializing rom resources """
        
        self.romController = controller
        self.engine = romController.getEngine()
        print(id(self.engine))
    ...
    def mainGameLoop(self):
        ...
        while not (keyboard.is_pressed('o') or (self.yIncrement is 0) or romController.getEndRom()):
            #print("pauseRom: {}".format(romController.getPauseRom()))
            #print("endRom: {}".format(romController.getEndRom()))
            if not self.romController.getPauseRom():
                if self.romController.getRomHalt() is True:
                    #unhalt rom
                    self.romController.setRomHalt(False)
            ...
            if (self.romController.getPauseRom() is True) and (not self.romController.getRomHalt()):
                #set rom to halted state
                self.romController.setRomHalt(True)
...
print(id(BrailleDisplay))
romController = valuePasser(pauseRomAddress, endRomAddress, BrailleDisplayAddress, romHaltAddress)
...
while romController.getEndRom() is False:
    rom.startMenu()
    rom.mainGameLoop()
    rom.scoreScreen()
else:
